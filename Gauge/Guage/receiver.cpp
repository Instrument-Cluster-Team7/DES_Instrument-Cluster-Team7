...
Receiver::Receiver(qreal * speed_ptr, QObject *parent):
    QThread(parent), socketCAN(socket(PF_CAN, SOCK_RAW, CAN_RAW)), ifr(), addr(), speed_ptr(speed_ptr){}
...
void Receiver::run(){
    ...
    while (true) {
        ...
        *speed_ptr = speed_data.speed_kmh;
